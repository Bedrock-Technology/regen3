...
func (_Staking *StakingCaller) GetMaxToStop(opts *bind.CallOpts) (*big.Int, error) {
	var out []interface{}
	err := _Staking.contract.Call(opts, &out, "getMaxToStop")
...
//
// Solidity: function getMaxToStop() view returns(uint256)
func (_Staking *StakingSession) GetMaxToStop() (*big.Int, error) {
	return _Staking.Contract.GetMaxToStop(&_Staking.CallOpts)
}

// GetMaxToStop is a free data retrieval call binding the contract method 0xbe9d96a0.
//
// Solidity: function getMaxToStop() view returns(uint256)
func (_Staking *StakingCallerSession) GetMaxToStop() (*big.Int, error) {
	return _Staking.Contract.GetMaxToStop(&_Staking.CallOpts)
}
...
type StakingClearedIterator struct {
	Event *StakingCleared // Event containing the contract specifics and raw log
...
// are any more events found. In case of a retrieval or parsing error, false is
// returned and Error() can be queried for the exact failure.
func (it *StakingClearedIterator) Next() bool {
	// If the iterator failed, stop iterating
	if it.fail != nil {
		return false
	}
...
// Error returns any retrieval or parsing error occurred during filtering.
func (it *StakingClearedIterator) Error() error {
	return it.fail
}

// Close terminates the iteration process, releasing any pending underlying
// resources.
func (it *StakingClearedIterator) Close() error {
	it.sub.Unsubscribe()
	return nil
}

// StakingCleared represents a Cleared event raised by the Staking contract.
type StakingCleared struct {
	Amount *big.Int
	Raw    types.Log // Blockchain specific contextual infos
}
...
func (_Staking *StakingFilterer) FilterCleared(opts *bind.FilterOpts) (*StakingClearedIterator, error) {

	logs, sub, err := _Staking.contract.FilterLogs(opts, "Cleared")
	if err != nil {
		return nil, err
	}
	return &StakingClearedIterator{contract: _Staking.contract, event: "Cleared", logs: logs, sub: sub}, nil
}
...
//
// Solidity: event Cleared(uint256 amount)
func (_Staking *StakingFilterer) WatchCleared(opts *bind.WatchOpts, sink chan<- *StakingCleared) (event.Subscription, error) {

	logs, sub, err := _Staking.contract.WatchLogs(opts, "Cleared")
	if err != nil {
		return nil, err
	}
	return event.NewSubscription(func(quit <-chan struct{}) error {
		...
		for {
			...
			case log := <-logs:
				// New log arrived, parse the event and forward to the user
				event := new(StakingCleared)
				if err := _Staking.contract.UnpackLog(event, "Cleared", log); err != nil {
					return err
				}
				event.Raw = log
...
//
// Solidity: event Cleared(uint256 amount)
func (_Staking *StakingFilterer) ParseCleared(log types.Log) (*StakingCleared, error) {
	event := new(StakingCleared)
	if err := _Staking.contract.UnpackLog(event, "Cleared", log); err != nil {
		return nil, err
	}
